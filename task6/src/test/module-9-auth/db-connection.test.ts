...
describe('Database connection', () => {
  ...
  test('Check if users exist in the table', async () => {
    const usersResponse = await pgClient.query('select * from users');

    expect(usersResponse.rows).toBeDefined();
    expect(Array.isArray(usersResponse.rows)).toBeTruthy();
    expect(usersResponse.rowCount).toBeGreaterThan(0);
  });
});
